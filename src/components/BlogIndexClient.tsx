            ...
            {featured && !activeTag && (
              <article className="feat">
                <div className="feat-img" aria-hidden="true">
                  <span className="mark-no">FEATURED · Nº {String(totalCount).padStart(2, '0')}</span>
                  <span className="mark">φ</span>
                  <span className="mark-label">ESSAY · {featured.displayDate.split(' ')[0]} MMXXVI</span>
                </div>
                <div className="feat-body">
                  ...
                  <Link href={`/writing/${featured.slug}`} className="feat-readon">
                    read the essay →
                  </Link>
                  <div className="feat-meta">
                    ...
                    {featured.tags[0] && (
                      <>
                        <span className="dot">●</span>
                        <span className="tag">{featured.tags[0].toUpperCase()}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              return (
                <section key={year} className="year">
                  <div className="year-label">
                    {year}
                    <span className="count">{yearPosts.length} piece{yearPosts.length !== 1 ? 's' : ''}</span>
                  </div>
                  <ul className="posts-list">
                    {yearPosts.map((p) => (
                      <ArticleCard
                        ...
                        date={p.displayDate}
                        readTime={p.readTime}
                        tags={p.tags.map((t) => ({ label: t, variant: tagVariant(t) as any }))}
                        pinned={p.pinned}
                      />
                    ))}
                  </ul>
          ...
          <aside className="rail">
            <div className="rail-box">
              ...
              <p>
                ...
                are re-edited after publishing; if a post feels different on
                your second read, that&apos;s why.
              </p>
              <p
                style={{
                  fontStyle: 'normal',
            ...
            <div className="subscribe">
              <h4>By email, if you prefer.</h4>
              <p>
                An essay a month, roughly. No tracking, no boilerplate, no
                &ldquo;welcome sequence.&rdquo;
              </p>
              <a
                href="mailto:saram.hassan1412@gmail.com?subject=Subscribe me to your writing"
                style={{
              ...
              <div className="ftn" style={{marginTop: '12px'}}>~ ONE EMAIL · MONTHLY ~</div>
            </div>
